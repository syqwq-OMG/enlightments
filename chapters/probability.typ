...
#theorem(title: [拉东-尼科迪姆定理 (The Radon-Nikodym Theorem)])[
  如果测度空间 $(X,Σ)$ 上的一个 $sigma$-有限测度 $nu$ 关于另一个 $sigma$-有限测度 $mu$ 绝对连续，那么存在一个在 $X$ 上的非负可测函数 $f:X to [0,oo)$，也就是 $mu$ 关于 $nu$ 的密度，或者 Radon-Nikodym 导数 $f=dif mu \/ dif nu$. 对所有的可测集合 $A$ ，都有：
  $
